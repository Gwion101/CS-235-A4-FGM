 ...
 * 
 *      The class will validate the received file and then store the attribute
 *      data in one single dimensional array with the data values in a 2
 *      dimensional array.
 */
...
public class DataSet {
	 ...
	 * @param m_columnNo
	 *            the desired column
	 * @return an array containing a column data values, the column of values
	 *         returned is chosen by the given argument integer which must be a
	 *         value between 0 and the number of attributes in the data set.
	 */
	 ...
	 * @param m_columnNo
	 *            the desired column
	 * @return a String of the 'nth' attribute of the attribute array 'nth' term
	 *         being the given argument integer which must be a value between 0
	 *         and the number of attributes in the data set.
	 */
	 ...
	 * @return TRUE on success if the .csv file is valid and the data set has
	...
	private Boolean checkForValidFile(File m_dataFile) {
		...
		if (!checkForCommas(m_dataFile)) {
			JOptionPane.showMessageDialog(m_frame,
					"File not compatible (contains no commas)", "File error",
		...
		if (!checkForAttributes(m_dataFile)) {
			JOptionPane.showMessageDialog(m_frame,
					"File not compatible (attribute error)", "File error",
					JOptionPane.ERROR_MESSAGE);
			return false;
		}
		...
		if (!checkForConsistentData(m_dataFile)) {
			JOptionPane.showMessageDialog(m_frame,
					"File not compatible (data not consistent)", "File error",
					JOptionPane.ERROR_MESSAGE);
			return false;
		}
		return true;
	...
	private int countAttributes(File m_dataFile) {
		...
		/*
		 * the record no is automatically the first entry so this var starts at
		 * 1
		 */
		m_delimiter = ",";
	...
	/**
	 * One dimensional object array containing the column data from the .csv
	 * file
	 */
